...
4. Verification — try all unique candidates against the target
"""
...
def strategy_verify(candidates, backend):
    """Strategy 4: Try all unique candidates against the target."""
    print("=" * 70)
    print("STRATEGY 4: VERIFICATION")
    print(f"Trying {len(candidates)} unique candidates against the target")
    print("=" * 70)

    found = []
    for candidate in candidates:
        success = backend.guess_password(candidate)
...
def run_analysis(acrostics, backend=None):
    ...
    if backend is None:
        # Fallback: import from gandalf directly (backwards compat)
        import os
        import sys
        ...
        class _LegacyBackend:
            ...
            def guess_password(self, pw, **kw):
                return guess_password(pw, cookies=self._cookies, **kw)

        backend = _LegacyBackend()

    strategy_verify(all_candidates, backend)
